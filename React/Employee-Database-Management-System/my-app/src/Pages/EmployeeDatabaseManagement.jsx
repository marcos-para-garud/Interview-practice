...
export default function EmployeeManagementSystem() {
  ...
  return (
    <div>
      <div>
      <h1 className="flex justify-center text-2xl m-4 font-bold">Employee Management System</h1>
      <button
      onClick={()=>setShowForm(!showForm)}
      className="bg-blue-600 mx-auto block m-4 text-white p-2 rounded">Add Details</button>
       <div className="flex justify-center gap-10 flex-wrap">
        <div className="border-2 p-4 block">
           {employees.length>0 && employees.map((i)=>
            {
              return(
                <li className="list-none border-2 p-2 m-2 w-60 hover:bg-gray-200 cursor-pointer">
       ...
       {showForm && (
        ...
        className="flex flex-col gap-2">
          <input type="text" value={newEmployee.name} onChange={(e)=>setNewEmployee({...newEmployee , name: e.target.value})} placeholder="Name" className="border p-2" />
          <input type="number" value={newEmployee.age} onChange={(e)=>setNewEmployee({...newEmployee , age: e.target.value})} placeholder="Age" className="border p-2" />
          <input type="text" value={newEmployee.designation} onChange={(e)=>setNewEmployee({...newEmployee , designation: e.target.value})} placeholder="Designation" className="border p-2" />
          <input type="text" value={newEmployee.department} onChange={(e)=>setNewEmployee({...newEmployee , department: e.target.value})} placeholder="Department" className="border p-2" />
          <input type="email" value={newEmployee.email} onChange={(e)=>setNewEmployee({...newEmployee , email: e.target.value})} placeholder="Email" className="border p-2" />
          <input type="text" value={newEmployee.phone} onChange={(e)=>setNewEmployee({...newEmployee , phone: e.target.value})} placeholder="Phone" className="border p-2" />
          <input type="text" value={newEmployee.address} onChange={(e)=>setNewEmployee({...newEmployee , address: e.target.value})} placeholder="Address" className="border p-2" />
          <input type="date" value={newEmployee.joiningDate} onChange={(e)=>setNewEmployee({...newEmployee , joiningDate: e.target.value})} className="border p-2" />
          <input type="number" value={newEmployee.salary} onChange={(e)=>setNewEmployee({...newEmployee , salary: e.target.value})} placeholder="Salary" className="border p-2" />
